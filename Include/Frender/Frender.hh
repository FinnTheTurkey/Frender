...
class LightRef
{
  public:
    ...
    LightRef(Renderer* renderer, LightTypes type, int32_t* index) : renderer(renderer), type(type), index(index){};
...
template <typename MeshT> struct MatSection
{
    MaterialRef mat;
...
class Renderer
{
  public:
    ...
    */
    LightRef createPointLight(glm::vec3 position, glm::vec3 color, float radius);

    /**
    Creates a direction light. Directional lights always apply to all elements of a scene
    */
    LightRef createDirectionalLight(glm::vec3 color, glm::vec3 direction);

    /**
    Returns the light's position
    */
    glm::vec3 getLightPosition(LightRef light);

    /**
    Sets the position of the light
    */
    void setLightPosition(LightRef light, glm::vec3 position);

    /**
    Gets the direction of a directional light
    */
    glm::vec3 getLightDirection(LightRef light);
  ...
  private:
    ...
    */
    ...
    Frender::RenderObjectRef _addRenderObject(std::vector<ShaderSection<MatSection<MeshSection<ROCpu, ROGpu>>>>& scene,
                                              ROCpu cpu, ROGpu gpu, GLTools::Shader shader, MeshRef mesh, uint32_t mat,
                                              glm::mat4 transform, int rows = 8);

    template <typename ROCpu, typename ROGpu>
    void _destroyRenderObject(std::vector<ShaderSection<MatSection<MeshSection<ROCpu, ROGpu>>>>& scene,
                              RenderObjectRef ro);
};
} // namespace Frender
